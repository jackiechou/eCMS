...
namespace Eagle.Common.Data
{
    public class SQLHandler
    {
        ...
        private string _objectQualifier = SystemSettings.ObjectQualifer;
        private string _databaseOwner = SystemSettings.DataBaseOwner;
        private string _connectionString = SystemSettings.ConnectionString;
        ...
        /// <param name="OutPutParamerterName">Accept Output Key Value Collection For Parameters</param>
        /// <returns></returns>
        public List<object> ExecuteNonQueryWithOuputList(string StroredProcedureName, List<KeyValuePair<string, object>> InParaMeterCollection, List<string> OutParaMeterNameCollection)
        {
            SqlConnection SQLConn = new SqlConnection(this._connectionString);
            try
            {
                ...
                SQLCmd.CommandType = CommandType.StoredProcedure;
                //Loop for Paramets
                for (int i = 0; i < InParaMeterCollection.Count; i++)
                {
                    SqlParameter sqlParaMeter = new SqlParameter();
                    sqlParaMeter.IsNullable = true;
                    sqlParaMeter.ParameterName = InParaMeterCollection[i].Key;
                    sqlParaMeter.Value = InParaMeterCollection[i].Value;
                    SQLCmd.Parameters.Add(sqlParaMeter);
                }
                //End of for loop


                foreach (var item in OutParaMeterNameCollection)
                {
                    SQLCmd.Parameters.Add(new SqlParameter(item, SqlDbType.Int));
                    SQLCmd.Parameters[item].Direction = ParameterDirection.Output;
                }
                ...
                foreach (var returnedItem in OutParaMeterNameCollection)
                {
                    returnedList.Add(SQLCmd.Parameters[returnedItem].Value);
                }
                SQLConn.Close();
                return returnedList;
            }
        ...
        public DataSet ExecuteAsDataSet(string StroredProcedureName, Hashtable parms)
        {
            SqlConnection SQLConn = new SqlConnection(this._connectionString);
            try
            {
        ...
        public string ExecuteScalar(string spName, params SqlParameter[] param)
        {
            SqlConnection SQLConn = new SqlConnection(this._connectionString);
            SQLConn.Open();
            SqlCommand SQLCmd = new SqlCommand();
            ...
            try
            {
                strRe = SQLCmd.ExecuteScalar().ToString();
                SQLConn.Close();
            }
            catch (Exception e)
            {
