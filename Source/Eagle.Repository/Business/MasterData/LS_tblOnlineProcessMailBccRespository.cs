...
{
    public class LS_tblOnlineProcessMailBccRespository
    {
       public EntityDataContext context { get; set; }

       public LS_tblOnlineProcessMailBccRespository(EntityDataContext context)
       { 
       ...
       public static List<Dictionary<string, object>> GetEmpJsonListById(int DMQuiTrinhID)
       {
           ...
           {
               var list = (from c in context.LS_tblOnlineProcessMailBcc
       ...
       public static IQueryable<LS_tblOnlineProcessMailBcc> Find(int DMQuiTrinhID, int EmpID)
       {
           using (EntityDataContext context = new EntityDataContext())
           {
               var query = context.LS_tblOnlineProcessMailBcc.Where(p => p.DMQuiTrinhID.Equals(DMQuiTrinhID)
       ...
       public static bool IsExisted(int DMQuiTrinhID, int EmpID)
        {
           LS_tblOnlineProcessMailBcc query = Find(DMQuiTrinhID, EmpID).SingleOrDefault();
           return (query != null);
        }
        ...
        {            
            using (EntityDataContext context = new EntityDataContext())
            {
                ...
                try
                {                   
                    LS_tblOnlineProcessMailBcc entity = new LS_tblOnlineProcessMailBcc();
        ...
        public static bool DeleteData(int DMQuiTrinhID)
        {
            using (EntityDataContext context = new EntityDataContext())
            {
                ...
                try
                {
                    List<LS_tblOnlineProcessMailBcc> lst = context.LS_tblOnlineProcessMailBcc.Where(p => p.DMQuiTrinhID.Equals(DMQuiTrinhID)).ToList();
                    if (lst.Count > 0)
                    {
